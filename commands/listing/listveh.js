...
module.exports = {
    name: "listveh",
    category: "listing",
    description: "Creates a listing embed for a vehicle",
    run: async (client, message, args) => {
        ...
        async function questionOne(channy) {
            channy.send(`**What are we listing** Use \`NA\` to disable.`).then(daMessage => {
                channy.awaitMessages(filter, {max: 1, time: ms('1h'), errors: ['time']}).then(async (ans) => {
        ...
        async function questionTwo(channy) {
            channy.send(`**Do you have a webstore or product showcase video? If so link it here.** Use \`NA\` to disable.`).then(daMessage => {
                channy.awaitMessages(filter, {max: 1, time: ms('1h'), errors: ['time']}).then(async (ans) => {
        ...
        async function questionFour(channy) {
            channy.send(`**Gallery link. This must be an image link!** Use \`NA\` to disable.`).then(daMessage => {
                channy.awaitMessages(filter, {max: 1, time: ms('1h'), errors: ['time']}).then(async (ans) => {
        ...
        async function questionFive(channy) {
            channy.send(`**What would you like to set as the main image? This must be an image link!** Use \`NA\` to disable.`).then(daMessage => {
                channy.awaitMessages(filter, {max: 1, time: ms('1h'), errors: ['time']}).then(async (ans) => {
        ...
        async function questionSix(channy) {
            channy.send(`**What is the price of the vehicle?**`).then(daMessage => {
                channy.awaitMessages(filter, {max: 1, time: ms('1h'), errors: ['time']}).then(async (ans) => {
        ...
        async function questionEight(channy) {
            channy.send(`**Give a description of the vheicle?** Use \`NA\` to disable.`).then(daMessage => {
                channy.awaitMessages(filter, {max: 1, time: ms('1h'), errors: ['time']}).then(async (ans) => {
        ...
        async function createEmbed(channy) {
            ...
            if(q6.toLowerCase() !== "na") {
                embed.setDescription(`> Want to credits? ${q7}\n> PRICE: USD$${q6}`)
            }
